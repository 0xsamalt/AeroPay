...
// Ethers custom signer wrapping session key
...
export function useSessionKey() {
  ...
  // Load key from localStorage on mount or address change
  useEffect(() => {
    if (!address) {
  ...
  const authorizeSession = useCallback(
    async (limitUSDC: string, expiryDays: number = 7) => {
      ...
      try {
        ...
        // Convert limit (USDC has 6 decimals)
